...
async def create_sale(sale: SalesSchema, db: Session = Depends(get_db)) ->JSONResponse:
    ...
    # get product
    product = db.query(all_models.Product).filter(all_models.Product.product_id == sale.product_id).first()
    if product is None:
        raise HTTPException(
    ...
    # get inventory
    inventory = db.query(all_models.Inventory).filter(all_models.Inventory.product_id == sale.product_id).first()
