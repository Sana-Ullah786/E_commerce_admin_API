...
async def analyze_sales(
    ...
    if category_id:
        product_ids_for_category = [product.product_id for product in db.query(all_models.Product.product_id).filter(all_models.Product.category_id == category_id).all()]
        query = query.filter(all_models.Sale.product_id.in_(product_ids_for_category))
    if merchant_id:
        product_ids_for_merchant = [product_merchant.product_id for product_merchant in db.query(all_models.ProductMerchant.product_id).filter(all_models.ProductMerchant.merchant_id == merchant_id).all()]
    ...
    return {"period": period, "total_sales": total_sales, "total_revenue": total_revenue}
