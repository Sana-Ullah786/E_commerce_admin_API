...
async def get_sales_by_category(
    ...
    """
    product_ids = [product.product_id for product in db.query(all_models.Product.product_id).filter(all_models.Product.category_id == category_id).all()]
    
    query = db.query(all_models.Sale).filter(all_models.Sale.product_id.in_(product_ids))
...
async def get_sales_by_merchant(
    ...
    """
    product_ids = [product_merchant.product_id for product_merchant in db.query(all_models.ProductMerchant.product_id).filter(all_models.ProductMerchant.merchant_id == merchant_id).all()]
