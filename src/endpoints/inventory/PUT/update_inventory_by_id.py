...
@router.put("/{inventory_id}", status_code=status.HTTP_201_CREATED, response_model=None)
async def update_inventory_by_id(inventory_id: int = Path(ge=0), db: Session = Depends(get_db),  new_stock: int = 0) ->JSONResponse:
    ...
    inventory = db.query(all_models.Inventory).filter(all_models.Inventory.inventory_id == inventory_id).first()
    if not inventory:
        raise HTTPException(
