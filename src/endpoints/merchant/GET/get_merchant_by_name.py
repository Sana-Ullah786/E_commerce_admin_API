...
async def get_merchant_by_name(merchant_name: str, db: Session = Depends(get_db)) ->JSONResponse:
    
    # get merchant by name case insensitive
    merchant = db.query(all_models.Merchant).filter(all_models.Merchant.name.ilike(merchant_name)).first()
    if merchant is None:
        raise HTTPException(
